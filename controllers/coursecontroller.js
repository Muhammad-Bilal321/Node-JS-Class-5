...
const CourseController = {
  add: (req, res) => {
    ...
    if (errArr.length > 0) {
      ...
      res.status(200).send(SendResponse(true, "Data Added Successfully", obj));
    }
    res.send(obj);
  },
  edit: () => {},
  get: async (req, res) => {
    try {
      ...
      res.status(300).send(SendResponse(true, "",result)) 
    } catch (error) {
      res.status(300).send(SendResponse(false, "Internal Server Error",result)) 
    }
  },
  getById: (req, res) => {
